% modified 7/15/2012 by SS to run with an outer code negotiating 
% experimental parameters and appropriate filenames, run_expt.m
...
function StTripNtrials_SS(pars)
...
for k=1:ncon;result(k).names=fname;;
...
indcond = repmat(dfConds,numtrial,1);

% set up randperm matrix of 0s and 1s, fliplr on those trials
nFlips = floor(numtrial/2);
bFlip = [ones(nFlips,1) zeros(numtrial-nFlips,1)];
bFlip = bFlip(randperm(numtrial));
bFlip = logical(bFlip);

indcond(bFlip,:) = fliplr(indcond(bFlip,:));
...
%%    if condid==4 AP=audioplayer(steadyseq4.testseqshortDF4,fs);end;
      ...
      pause(.5);
%%      pause(5);
      play(APwarning);while isplaying(APwarning)pause(0.1);end;pause(.5);
      play(AP)
...
%%
  ...
  %  pause(60); % for long durn trials/sessions
    ...
    resultfname=strcat('~/Dropbox/my codes/rinzel/experiment_code/data/',...
        fname,'.mat');
