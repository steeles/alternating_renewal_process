...
if bSim
    [u1 u2 s1 s2 tax n1 n2 iboth pars] = noise_adaptation;
     ...
     pars.Gamma = 0;
    [u1 u2 s1 s2 tax n1 n2 iboth pars] = noise_adaptation(pars,1)
     pars.sig = .12; pars.t_in_seconds = 20;
     [u1 u2 s1 s2 tax n1 n2 iboth pars] = noise_adaptation(pars,1)
     pars.Gamma = .7; pars.sig = .06;
     [u1 u2 s1 s2 tax n1 n2 iboth pars] = noise_adaptation(pars,1)
    
    pars.sig = .06; pars.t_in_seconds = 500; pars.tau_slow = 200;
...
for cInd = 4%1:NumCond
   for sInd = 7%1:NumSubj
       ...
       % other
       IntProp = mu1/(mu1+mu2)
       figure; histfit(allDurs(allDurs(:,1)~=0,1),10,'gamma')
...
%     
    ...
    compute_cumhist_corr(DursCell,tau);
       ...
       if bSim
           title(sprintf('G=%.1f, noise=%.2f, tauA=%.1f, N=%d',...
               pars.Gamma,pars.sig,pars.tau_slow/100,length(T1)))
       else
