...
mnT2byH2 = mean(T2_by_H1,'omitnan');
stdT2 = std(T2_by_H1,'omitnan');
nT2 = sum(~isnan(T2_by_H1));
seT2 = stdT2./sqrt(nT2);
...
for ind = 1:length(goodFits)
    ...
    arpPred = mean(arpSort,1,'excludenan');
    
    chUB = chSort(tail,:);
    chLB = chSort(end-tail,:);
    cumhistPred = mean(chSort,1,'excludenan');
    ...
    mk_Nice_Plot; title(sprintf('u_1=%.2f,u_2=%.2f|(r1,r2)=%.
